// app/(main)/settings/createPin.tsx
...
export default function CreatePinScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await refreshUser();
      Alert.alert(t('security_page.biometrics_success_title'), t('create_pin_page.success_pin_created'));
      router.back();
    } catch (error) {
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.inputGroup}>
          <Text style={styles.label}>{t('create_pin_page.new_pin_label')}</Text>
          <TextInput style={styles.input} value={newPin} onChangeText={setNewPin} keyboardType="number-pad" maxLength={4} secureTextEntry />
        </View>
        <View style={styles.inputGroup}>
          <Text style={styles.label}>{t('create_pin_page.confirm_pin_label')}</Text>
        ...
        <View style={styles.inputGroup}>
          <Text style={styles.label}>{t('create_pin_page.pin_question_label')}</Text>
        ...
        <TouchableOpacity style={styles.button} onPress={handleSave} disabled={isSaving}>
          {isSaving ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>{t('create_pin_page.btn_save_pin')}</Text>}
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
